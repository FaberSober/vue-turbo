 ...
 * @param path
 */
function getRoutePath(path: string): any {
  return path;
}
...
export function getAllTabs(tabs: App.Global.Tab[], homeTab?: App.Global.Tab) {
  ...
  const fixedTabs = filterHomeTabs.filter((tab) => tab.fixedIndex !== undefined).sort((a, b) => a.fixedIndex! - b.fixedIndex!);
...
export function getTabByRoute(route: App.Global.TabRoute) {
  ...
  const tab: App.Global.Tab = {
    ...
    routeKey: name as FaRouter.RouteKey,
    routePath: path as FaRouter.RoutePath,
    fullPath,
 ...
 */
export function getDefaultHomeTab(router: Router, homeRouteName: FaRouter.RouteKey) {
 ...
 * @param tabs
 */
export function findTabByRouteName(name: FaRouter.RouteKey, tabs: App.Global.Tab[]) {
