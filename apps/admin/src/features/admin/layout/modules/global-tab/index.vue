...
import { FaPageTab } from '@fa/ui';
import BetterScroll from '@f/admin/components/custom/better-scroll.vue';
...
import { useTabStore } from '@f/admin/stores/tab';
    ...
    <div ref="bsWrapper" class="h-full flex-1-hidden">
      <BetterScroll ref="bsScroll" :options="{ scrollX: true, scrollY: false, click: appStore.isMobile }">
        <div ref="tabRef" class="h-full flex pr-18px" :class="[themeStore.tab.mode === 'chrome' ? 'items-end' : 'items-center gap-12px']">
          ...
          >
            <template #prefix>
              <FaSvgIcon :icon="tab.icon" :local-icon="tab.localIcon" class="inline-block align-text-bottom text-16px" />
            </template>
